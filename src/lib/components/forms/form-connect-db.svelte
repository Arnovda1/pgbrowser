...
<form method="POST" use:enhance class="grid gap-3 [@media(max-height:600px)]:grid-cols-2">
	...
	<Form.Field {form} name="host">
		<Form.Control>
			{#snippet children({ props })}
				...
				<Input {...props} bind:value={$formData.host} disabled={loading} placeholder="localhost" />
			{/snippet}
		</Form.Control>
		<Form.FieldErrors />
	</Form.Field>
	...
	<Form.Button
		class="w-full [@media(max-height:600px)]:col-span-2"
		onclick={loading ? stopConnecting : undefined}
	>
		{#if loading}
			<SquareIcon />
			Stop connecting
