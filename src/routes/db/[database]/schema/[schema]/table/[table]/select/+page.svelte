<script lang="ts">
	...
	const fetchRecords = async () => {
		result = await getRecords(page.params);
	}
	...
	const loadMore = async () => {
		...
		const res = await getRecords(page.params, result.data.length);
	...
	$effect(() => {
		if (page.params.table) fetchRecords();
	});
...
{#if result}
	<div class="flex gap-2 items-center">
		<div class="border w-fit shrink-0 rounded-xl overflow-hidden opacity-70 pr-1">
...
<Resizable.PaneGroup direction="horizontal">
  <Resizable.Pane defaultSize={100} minSize={50} maxSize={100}>
		<div class="grid min-h-0 h-full">
			<RecordsTable
				{result} {loadMore} {primaryKey} {onRecordClick} showButtons
	...
	<Resizable.Handle class='hover:bg-primary my-4 {sidePane?.isExpanded() ? 'mx-2.5' : ''}' />

  <Resizable.Pane
		minSize={15} defaultSize={30} maxSize={50}
		bind:this={sidePane}
		collapsible
	>
    <JsonViewer
			{...page.params} {primaryKey} {sidePane}
			pkValue={selectedRecord}
		/>
  </Resizable.Pane>
</Resizable.PaneGroup>